...
TEXT ·p256PointAddAffineAsm(SB),0,$264-96
	...
	// Begin point add
	LDx(z1in)
	CALL	sm2SqrInternal<>(SB)    // z1²
	STy(z1sqr)

	LDx(x2in)
	CALL	sm2MulInternal<>(SB)    // x2 * z1²
	...
	LDP	4*16(a_ptr), (acc0, acc1)// if select[0] == 0, z3 = z1
	...
	ANDS	$2, hlp1, ZR            // if select[1] == 0, z3 = 1
	CSEL	EQ, acc0, y0, y0
	CSEL	EQ, acc1, y1, y1
	CSEL	EQ, acc2, y2, y2
	CSEL	EQ, acc3, y3, y3
	...
	LDy(z1sqr)
	CALL	sm2MulInternal<>(SB)    // z1³

	LDx(y2in)
	CALL	sm2MulInternal<>(SB)    // s2 = y2 * z1³
	STy(s2)
	...
	CALL	sm2SqrInternal<>(SB)    // rsqr = r²
	STy	(rsqr)

	LDx(h)
	CALL	sm2SqrInternal<>(SB)    // hsqr = h²
	STy(hsqr)

	CALL	sm2MulInternal<>(SB)    // hcub = h³
	STy(hcub)

	LDx(y1in)
	CALL	sm2MulInternal<>(SB)    // y1 * h³
	STy(s2)
	...
	CALL	sm2MulInternal<>(SB)    // u1 * h²
	STP	(y0, y1), h(0*8)
	STP	(y2, y3), h(2*8)

	p256MulBy2Inline               // u1 * h² * 2, inline

	LDy(rsqr)
	CALL	sm2SubInternal<>(SB)    // r² - u1 * h² * 2
...
TEXT ·p256PointAddAsm(SB),0,$392-80
	...
	// Begin point add
	LDx(z2in)
	CALL	sm2SqrInternal<>(SB)    // z2²
	STy(z2sqr)

	CALL	sm2MulInternal<>(SB)    // z2³

	LDx(y1in)
	CALL	sm2MulInternal<>(SB)    // s1 = z2³*y1
	STy(s1)
	...
	LDx(y2in)
	CALL	sm2MulInternal<>(SB)    // s2 = z1³*y2
	...
	LDx(z2sqr)
	LDy(x1in)
	CALL	sm2MulInternal<>(SB)    // u1 = x1 * z2²
	STy(u1)

	LDx(z1sqr)
	LDy(x2in)
	CALL	sm2MulInternal<>(SB)    // u2 = x2 * z1²
	STy(u2)
