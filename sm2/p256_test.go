...
func TestRRbyP256(t *testing.T) {
	...
	ww = N0.Bits()
	ck := calcK0(n)
	t.Logf("K0: %s (%x), n*K0: %x %x %x %x", K0.Text(16), ck,
		ww[0], ww[1], ww[2], ww[3])
	ck = calcK0a(p)
	t.Logf("K0a P: %x", ck)
	K0.SetUint64(1)
...
func TestRRbyBTC(t *testing.T) {
	...
	ww = N0.Bits()
	ck = calcK0(p)
	t.Logf("K0: %s (%x), p*K0: %x %x %x %x", K0.Text(16), ck,
		ww[0], ww[1], ww[2], ww[3])
	ww = p.Bits()
	t.Logf("P: %X %X %X %X", ww[0], ww[1], ww[2], ww[3])
	ww = cParams.Gx.Bits()
...
func TestSM2AsmGoGMult(t *testing.T) {
	goCurve := SM2go()
	//asm version SM2 works
	asmCurve := SM2()
	//asmCurve := P256()
	gx, gy := goCurve.ScalarBaseMult(d1.Bytes())
	ax, ay := asmCurve.ScalarBaseMult(d1.Bytes())
	if ax.Cmp(gx) != 0 {
		t.Logf("ECG mult X diff:\n%s vs\n%s", gx.Text(16), ax.Text(16))
		t.Fail()
	}
	if ay.Cmp(gy) != 0 {
		t.Logf("ECG mult Y diff:\n%s vs\n%s", gy.Text(16), ay.Text(16))
		t.Fail()
	}
}
